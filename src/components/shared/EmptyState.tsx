...
const EmptyState: React.FC<EmptyStateProps> = ({
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-4">
      <div className="w-20 h-20 rounded-full bg-[#F5F5F5] dark:bg-[#1A1A1A] flex items-center justify-center mb-4">
        <SearchX size={32} className="text-[#6B7280] dark:text-gray-500" />
      </div>
      <h3 className="text-lg font-semibold text-[#111111] dark:text-white mb-2">{title}</h3>
      <p className="text-sm text-[#6B7280] dark:text-gray-400 mb-6 text-center max-w-sm">{description}</p>
      {onAction && (
        <Button
          onClick={onAction}
